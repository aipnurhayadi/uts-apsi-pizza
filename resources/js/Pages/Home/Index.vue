...
watch(productTabActive, () => {
    productTabActiveData.value =
        props.products.find((r) => `product-${r.id}` == productTabActive.value) ?? {};
});
</script>
<template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="overflow-hidden bg-white p-5 shadow-sm sm:rounded-lg">
                    <n-grid x-gap="24" :cols="12" class="flex h-full items-center justify-center">
                        <n-gi :span="8">
                            <div class="mb-5">
                                <Link :href="route('addresses.edit')">
                            ...
                            <span v-if="props.products">
                                <n-tabs placement="bottom" justify-content="start" animated class="w-100"
                                    ...
                                    <n-tab-pane v-for="(
                                            product, product_idx
                                        ) in props.products" :key="product_idx" :name="`product-${product.id}`">
                                        <template #tab>
                            ...
                            <span v-else>
                                <i class="text-center text-gray-400">Belum ada produk</i>
                            </span>
                        </n-gi>
